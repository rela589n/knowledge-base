**Information Expert** - the **logic** *must be **implemented in*** the conceptually **proper classes** for it
	(those **having data** needed to fulfill that logic).

> **Example**: **`Sale`** and **`LineItem`** classes 
> 	(`sale.lineItems` relationship),
> We'd *put* **`getTotalPrice`** method ***into* `Sale`** class
> 	(since it's the owner of `lineItems` relationship),
> rather than anywhere else.

It is similar to **knowledge of expert** in the real world.
You (ui) need to build a house (business logic),
you ask architect (Expert) to do the job,
