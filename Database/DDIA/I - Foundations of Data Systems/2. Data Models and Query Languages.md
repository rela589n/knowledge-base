...
Complex applications are layered.
Each layer hides complexity of layer below making it easier to understand the concept as a whole.
...
[[NoSQL]] - retroactively called as Not Only SQL.
...
Objects in applications are not of the same structure as database tables. Translation layer is required here to work properly. Such disconnect is called _impedance mismatch_.
...
Main points of Document-based are: schema flexibility, better performance due to data locality and structure, may be closer to application structures.
...
Today relational databases have support for both json and xml columns (with features like query inside of them and local modifications). On the other hand, document databases already added support for joins (something like `$lookup`  aggregation in [[MongoDB]]). This way, databases complement each other.
